...
public sealed partial class SqlInsertBuilder
{
    ...
    public SqlInsertBuilder Output<TTable>(Expression<Func<TTable, object>>? columnSelector = null)
    {
        if (_output is null)
        {
    ...
    /// <summary>
    /// Creates or appends an OUTPUT clause that outputs all columns from the INSERTED table.
    /// </summary>
    /// <returns>Instance of self.</returns>
    public SqlInsertBuilder OutputInserted()
    {
        if (_output is null)
        {
            _output = SqlOutput.WithAllColumns(SqlOutput.EphermeralTableName.INSERTED);
        }
        else
        {
            _output.AddAllColumns(SqlOutput.EphermeralTableName.INSERTED);
        }
    ...
    /// <summary>
    /// Creates or appends an OUTPUT clause that outputs the specified columns from the INSERTED table.
    /// </summary>
    ...
    /// <returns>Instance of self.</returns>
    public SqlInsertBuilder OutputInserted(params IEnumerable<string> columnNames)
    {
        if (_output is null)
        {
            _output = SqlOutput.WithColumns(SqlOutput.EphermeralTableName.INSERTED, columnNames);
        }
        else
        {
            _output.AddColumns(SqlOutput.EphermeralTableName.INSERTED, columnNames);
        }
    ...
    /// <returns>Instance of self.</returns>
    public SqlInsertBuilder OutputInserted(Dictionary<string, string> columnNamesWithAliases)
    {
        if (_output is null)
        {
    ...
    /// <returns>Instance of self.</returns>
    public SqlInsertBuilder OutputToTable<TOutputTable>()
    {
        if (_output is null)
        {
    ...
    public SqlInsertBuilder OutputToTable(string tableName)
    {
        if (_output is null)
        {
            throw new InvalidOperationException("Set the output columns before specifying an output table.");

...
/// <summary>
/// Causes an INSERT action to be written to the parent WHEN clause of the MERGE statement.
/// </summary>
...
public sealed class InsertAction<TTarget, TSource>
{
    /// <summary>
    /// Insert values in the table mapped to <typeparamref name="TTarget"/> by from mapped values from the table pointed to by <typeparamref name="TSource"/>, 
    ...
    ///                                 { "Price", ot => ot.Price },
    ///                                 { "Status", ot => ot.Status },
    ///                                 { "LastModified", ot => DateTime.UtcNow },
    ///                                 { "IsActive", ot => true }
    ///                             },
    ...
    ///             
    ///     [SQL]:
    ///         INSERT ([Id], [Name], [Price], [Status], [LastModified], [IsActive], [IsDeleted])  
    ///             VALUES ([SRC].[Id], [SRC].[Name], [SRC].[Price], [SRC].[Status], GETUTCDATE(), 1, 0)
    /// </example>
    public WhenBuilder<TTarget, TSource> Set(
        ...
        )
    {
        ...
        {
            if (member.PersistenceInfo is SqlTablePopulatedColumn)
            {
                continue;
            }
            ...
            if (columnMappings.ContainsKey(member.PersistenceInfo.TableFieldName))
            {
                ...
                {
                    throw new InvalidOperationException($"The column '{member.PersistenceInfo.CreateQualifiedName()}' has already been mapped for insert.");
                }
        ...
        if ((additionalValues is not null) && (additionalValues.Count > 0))
        {
            foreach (string additionalColumnName in additionalValues.Keys)
            {
                ...
                {
                    throw new InvalidOperationException($"The column '{additionalColumnName}' has already been mapped for insert.");
                }
        ...
        _mergeBuilder.Write((new StringBuilder())
                .Append("INSERT (").AppendJoin(", ", values.Keys).Append(") VALUES (")
                    .AppendJoin(", ", values.Values).Append(')').Append(' '));
    ...
    internal InsertAction(SqlMergeBuilder<TTarget> mergeBuilder, WhenBuilder<TTarget, TSource> whenBuilder)
    {

...
/// <summary>
/// Extends the <see cref="ListValueBinderBase{TControl, TValue}"/>, for the <see cref="ListBox"/> control.
/// </summary>
/// <typeparam name="TValue">The type of values that would be a part of the control's presentation.</typeparam>
internal sealed class ListBoxBinder<TValue> : ListValueBinderBase<ListBox, TValue>
{
    ...
    public override void BindControl(object dataContext)
    {
        ...
        //TODO: When dataContext.GetType() != TValue, the below code has the same bug as 
        //      there was in ComboBoxBinder. Find and solve it whenever we use a LB binding next!
        /*  Scenario:
         *      Form is bound to TEntity = Person
         *      LB is bound to {Hobby[]}
         *      
        ...
        foreach (TValue val in values)
        {
            ...
            if (index >= 0)
            {
                ControlPartner.SetSelected(index, true);
            }
        }
    }
    ...
    public override void BindEntityMember(object dataContext)
    {
        ...
        foreach (object? item in ControlPartner.SelectedItems)
        {
            if (item is null)
            {
    ...
    /// </summary>
    /// <param name="control">The control of type <see cref="ListBox"/> participating in the binding.</param>
    ...
    internal ListBoxBinder(ListBox control, PersistenceContainerMemberInfo member,
    ...
    /// <param name="bindingDirection">The direction of binding.</param>
    internal ListBoxBinder(ListBox control, PersistenceContainerMemberInfo member, Dictionary<string, TValue> valueSource, BindingDirection bindingDirection = BindingDirection.TwoWay)
    ...
    /// <param name="valueMember">The property of <typeparamref name="TValue"/> that is to be used as the member item's internal value.</param>
    /// <param name="bindingDirection">The direction of binding.</param>
    internal ListBoxBinder(ListBox control, PersistenceContainerMemberInfo member, Control bindOnlyWhenChanged,
